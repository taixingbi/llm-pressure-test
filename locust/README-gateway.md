...
Stream: TTFT = first `delta.content` chunk (skips gateway `conversation` + empty role chunk); full = through `data: [DONE]`. Locust header-only timing is corrected for full drain.
